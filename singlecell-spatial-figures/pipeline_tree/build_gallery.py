#!/usr/bin/env python3
"""Build inspectable synthetic examples for all 36 executable pipeline modules."""
...
import runtime_fixtures4 as f4
import runtime_fixtures5 as f5
...
def all_fixtures():
    ...
    for pack in (f2,f3,f4,f5):
        for mid,fn in pack.FIXTURES.items():out[mid]=fn()
    return out


def build(root:Path):
    root.mkdir(parents=True,exist_ok=True);rows=[];fixtures=all_fixtures()
    missing=set(run_pipeline.IMPLEMENTED["modules"])-set(fixtures);extra=set(fixtures)-set(run_pipeline.IMPLEMENTED["modules"])
    ...
    for mid in sorted(run_pipeline.IMPLEMENTED["modules"]):
        inputs,cfg=fixtures[mid];folder=root/mid.replace(".","__");manifest=run_pipeline.render_module(mid,inputs,cfg,folder);targets=[x["target"] for x in manifest["generated"]];skipped=[x["target"] for x in manifest["skipped"]];declared=run_pipeline.IMPLEMENTED["modules"][mid];missing_targets=(set(declared["required_targets"])|set(declared["advanced_targets"]))-(set(targets)|set(skipped))
        if missing_targets:raise ValueError(f"{mid}: declared targets neither generated nor explicitly skipped: {sorted(missing_targets)}")
        rows.append({"module_id":mid,"generated":len(targets),"skipped":len(skipped),"targets":targets,"folder":folder.name})
