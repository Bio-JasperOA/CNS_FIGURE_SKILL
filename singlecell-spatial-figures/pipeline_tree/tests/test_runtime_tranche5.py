...
router=load("run_pipeline");fixtures=load("runtime_fixtures5")

@pytest.mark.parametrize("module_id",sorted(fixtures.FIXTURES))
def test_tranche5_real_renderers(module_id,tmp_path):
...
def test_all_36_modules_are_executable():
    roadmap=json.loads((HERE/"MODULE_REGISTRY.json").read_text())
    assert len(roadmap["modules"])==36
    assert set(router.IMPLEMENTED["modules"])=={m["id"] for m in roadmap["modules"]}
    assert set(router.DISPATCH)==set(router.IMPLEMENTED["modules"])
