...
fixtures=load("runtime_fixtures")


@pytest.mark.parametrize("module_id", sorted(fixtures.FIXTURES))
def test_runtime_generates_figures(module_id,tmp_path):
    table,cfg=fixtures.FIXTURES[module_id]()
    manifest=runtime.render_module(module_id,{"main":table},cfg,tmp_path/module_id.replace(".","_"))
    ...
    for rec in manifest["generated"]:
        outs=rec["outputs"]
        assert Path(outs["png"]).is_file()
        assert Path(outs["pdf"]).is_file()
        if rec["kind"]!="review_contact_sheet" and rec["engine"]!="alias":
            assert "svg" in outs and Path(outs["svg"]).is_file()
...
def test_real_data_requires_explicit_provenance(tmp_path):
    table,cfg=fixtures.scrna_qc()
    cfg=dict(cfg);cfg["demo"]=False;cfg.pop("provenance",None)
    with pytest.raises(ValueError):
        runtime.render_module("scrna.qc",{"main":table},cfg,tmp_path/"x")


def test_legacy_subtitle_is_ignored(tmp_path):
    table,cfg=fixtures.scrna_qc()
    cfg=dict(cfg);cfg["subtitle"]="THIS MUST NOT RENDER"
    manifest=runtime.render_module("scrna.qc",{"main":table},cfg,tmp_path/"x")
    assert "THIS MUST NOT RENDER" not in json.dumps(manifest)
