...
def demo(out):
    ...
    for kind,(d,c) in generate().items():
        ...
        c.pop('subtitle',None);_write(csv.with_suffix('.json'),c)
        reports={}
    ...
    d,c=high_capacity();d.to_csv(root/'source_data'/'embedding_100.csv',index=False);_write(root/'source_data'/'embedding_100.json',c)
    c.pop('subtitle',None);_write(root/'source_data'/'embedding_100.json',c)
    for mode in ['minimal','advanced']:
        export(render('embedding',d,c,mode),root/'figures'/f'embedding_100_{mode}')
    _write(root/'manifest.json',{'version':VERSION,'data_kind':'synthetic_style_fixture','subtitle_policy':'forbidden','pairs':manifest})
...
def main():
    ...
    if args.cmd=='demo':demo(args.out)
        ...
        _write(Path(args.out).with_suffix('.provenance.json'),{'input_sha256':_hash(args.input),'config_sha256':_hash(args.config),'version':VERSION,'demo':bool(c.get('demo',False))})
